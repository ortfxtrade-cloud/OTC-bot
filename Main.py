...
def analyze_market(asset):
    ...
    1. Check 15min trend (price vs EMA200)
    2. Check 2min timeframe for MACD cross
    3. Check RSI conditions
    """
    
    # Step 1: Check 15-minute trend
    trend_15m = check_15min_trend(asset)
    
    if not trend_15m['valid']:
        return {
            "signal": None,
            "reason": f"15m trend not valid: {trend_15m['reason']}",
            "trend_15m": trend_15m
        }
    ...
    if trade_direction == "BUY":
        signal_type = "CALL"
        expected_cross = "BULLISH_CROSS"  # MACD crossing UP
    else:  # SELL
        signal_type = "PUT"
        expected_cross = "BEARISH_CROSS"  # MACD crossing DOWN
    
    # Step 2: Check 2-minute timeframe for MACD cross
    cross_result = check_2min_macd_cross(asset, expected_cross)
    
    if not cross_result['cross_detected']:
        return {
            "signal": None,
            "reason": f"No valid MACD cross on 2min: {cross_result['reason']}",
            "trend_15m": trend_15m,
            "cross_result": cross_result
        }
    
    # Step 3: Check RSI on 2-minute timeframe
    rsi_valid, rsi_msg = check_rsi_conditions(cross_result['rsi'], signal_type)
    
    if not rsi_valid:
        return {
            "signal": None,
            "reason": f"RSI condition not met: {rsi_msg}",
            "trend_15m": trend_15m,
            "cross_result": cross_result,
            "rsi_result": {"valid": False, "message": rsi_msg, "rsi": cross_result['rsi']}
        }
...
def execute_trade(signal_id):
    ...
    try:
        ...
        # Execute the trade with 2-minute expiry (120 seconds)
        buy_info = api.buy(1, asset, signal.lower(), 120)
        trade_id = buy_info.get("id")
        
        if trade_id:
            # Start monitoring thread (wait 2 minutes + buffer)
            threading.Thread(
...
def monitor_trade_result(trade_id, asset, signal_id):
    """Waits for the trade to finish and reports result. (2-minute expiry)"""
    time.sleep(125)  # Wait for 2-minute trade duration (120s + 5s buffer)
...
Use /menu for more options
        ...
        btn_details = types.InlineKeyboardButton("📈 History", callback_data=f"history")
        btn_back = types.InlineKeyboardButton("🔙 Menu", callback_data="main_menu")
        markup.add(btn_details, btn_back)
...
def send_trade_signal(asset, analysis):
    """Send trade signal for approval with strategy breakdown"""
    ...
    # Format expiry time (2 minutes from now)
...
def expire_signal(signal_id):
    ...
    if signal_id in pending_signals and pending_signals[signal_id]['status'] == 'pending':
        ...
        expire_msg = f"⏰ *Signal Expired*\n\nAsset: `{asset}`\nSignal ID: `{signal_id[:8]}...`\n\nThis signal has expired. New signals will be sent when available."
...
Trade placed. Results in 2 minutes.
        ...
        btn_track = types.InlineKeyboardButton("📊 Track", callback_data=f"track_{result}")
        btn_menu = types.InlineKeyboardButton("🔙 Menu", callback_data="main_menu")
        markup.add(btn_track, btn_menu)
...
• Histogram: {ind['macd']['MACDh_12_26_9'].iloc[-1]:.6f}
...
⏱️ *Signal Valid For:* {signal_timeout - (time.time() - signal_data['timestamp']):.0f}s
"""
    
    markup = types.InlineKeyboardMarkup(row_width=2)
    btn_back = types.InlineKeyboardButton("🔙 Back", callback_data=f"back_to_signal_{signal_id}")
    btn_trend = types.InlineKeyboardButton("📈 15m Chart", callback_data=f"trend_{signal_id}")
    markup.add(btn_back, btn_trend)
...
• RSI (14): {ind['rsi']:.2f}
• MACD: {ind['macd']['MACD_12_26_9'].iloc[-1]:.6f}
• Signal: {ind['macd']['MACDs_12_26_9'].iloc[-1]:.6f}

*Analysis:*
{trend_15m['reason']}

*Price vs EMA200:* {trend_15m.get('price_vs_ema_percent', 0):.2f}%
"""
    
    markup = types.InlineKeyboardMarkup(row_width=2)
    btn_back = types.InlineKeyboardButton("🔙 Back", callback_data=f"back_to_signal_{signal_id}")
    btn_details = types.InlineKeyboardButton("📊 Details", callback_data=f"details_{signal_id}")
    markup.add(btn_back, btn_details)
...
📨 *Pending Signals:* {len([s for s in pending_signals.values() if s['status'] == 'pending'])}
⏱️ *Last Updated:* {time.strftime("%H:%M:%S")}

*Strategy:*
1️⃣ 15min Trend (Price vs EMA200)
2️⃣ 2min MACD Cross
3️⃣ RSI Confirmation (30-45 BUY / 55-70 SELL)
⏱️ *Expiry:* 2 MINUTES
...
def show_status(message):
    ...
    executed_today = len([s for s in pending_signals.values() if s['status']import os
import time
import threading
from flask import Flask
import pandas as pd
...
*Status:* {"✅ Running" if bot_running.is_set() else "⏸️ Stopped"}
*Strategy:* 15m Trend + 2min MACD Cross + RSI
*Expiry:* 2 MINUTES
...
def run_trading_engine():
    if api.connect():
        ...
        markup = get_main_keyboard()
        tg_bot.send_message(chat_id, startup_msg, parse_mode="Markdown", reply_markup=markup)
...
if __name__ == "__main__":
    ...
    app.run(host="0.0.0.0", port=port)￼Enter == 'executed'])
